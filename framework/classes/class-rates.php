...
/**
 * BetterRateHandling class for fetching EU Tax rates.
 *
 * @since 1.0.0
 */
class BetterRateHandling {
		...
		?>
			...
			( function( $ ) {
				$( document ).ready( function() {
					...
					var tax_description = ' <?php esc_attr_e('Description:', 'better-tax-handling');?> <input id="better-tax-whatdescription" title="<?php esc_attr_e('The description that will be used when using the button for mass adding/updating of EU rates', 'better-tax-handling'); ?>" type="text" size="6" value="Tax">';
	...
	public function get_tax_rates( $use_transient = true ) {
		...
		if ( empty( $new_rates ) && ( false != ( $rates_from_file = file_get_contents( dirname( __FILE__ ) . '/data/rates.json' ) ) ) ) {
